...
export const VideoGallery = ({ title, videos, sectionId = "works" }: VideoGalleryProps) => {
  ...
  const displayedVideos = isMobile
      ? videos
      : videos.slice(thumbnailStartIndex, thumbnailStartIndex + thumbnailsPerPage);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              {displayedVideos.map((video, index) => (
                  <motion.div
                      key={`${video.id}-${index}`}
                      className="relative w-72 flex-shrink-0 bg-gray-900 overflow-hidden cursor-pointer group rounded-lg shadow-lg snap-start"
                      whileHover={{ scale: 1.03 }}
                      transition={{ duration: 0.3 }}
                  >
                    {/* Autoplaying muted YouTube iframe */}
                    <iframe
                        src={`https://www.youtube.com/embed/${video.id}?autoplay=1&mute=1&loop=1&playlist=${video.id}&controls=0&modestbranding=1&playsinline=1`}
                        className="w-full h-full"
                        style={{ pointerEvents: "none", minHeight: "160px" }}
                    ...
                    <div
                        className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-300 flex flex-col justify-end p-3 sm:p-4 cursor-pointer"
